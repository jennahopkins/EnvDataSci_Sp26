---
title: "Lecture 1B. Data Validation"
...
The primary goal of QA is to scrub the errors from the input to the analytical pipeline.
...
```{r}
head(CO2) #first 6 rows
tail(CO2,10) #last 10 rows
...
The primary goal of QC is to scrub the errors from the output from the analytical pipeline.

1) Consider future data users:

- Do the metadata explicitly explain basic information about the data that can make tracking and working with the data easier?
...
## NOAA Data Example
We'll work with a simple dataset from the National Oceanic and Atmospheric Administration (NOAA). These data are the bimonthly values for the Multivariate El Niño Southern Oscillation Index (https://psl.noaa.gov/enso/mei/) or MEI for short. Our end goal is to create a time series showing the El Niño and La Niña events over the past 47 years.

First, let's load the input data. There are four versions of the data:

- raw data updated on March 10, 2025
- cleaned data updated on March 10, 2025
- raw data updated on January 8, 2026
- cleaned data updated on January 8, 2026

```{r}
MEI_raw_1 <- read.delim("MEI_raw_20250310.txt")
MEI1 <- read.table("MEI_cleaned_20250310.txt", header = TRUE)
MEI_raw_2 <- read.delim("MEI_raw_20260108.txt")
MEI2 <- read.table("MEI_cleaned_20260108.txt", header = TRUE)
```

Now take a look at the MEI_raw vs. MEI_cleaned files for the older version. In this case, it was easiest to work with the data in a text editor, but larger files might need to be manipulated in R.
```{r}
View(MEI_raw_1)
View(MEI1)
```
...
```{r}
MEI1$FM <- replace(MEI1$FM, MEI1$FM == -999, NA)
MEI1$MA <- replace(MEI1$MA, MEI1$MA == -999, NA)
MEI1$AM <- replace(MEI1$AM, MEI1$AM == -999, NA)
MEI1$MJ <- replace(MEI1$MJ, MEI1$MJ == -999, NA)
MEI1$JJ <- replace(MEI1$JJ, MEI1$JJ == -999, NA)
MEI1$JA <- replace(MEI1$JA, MEI1$JA == -999, NA)
MEI1$AS <- replace(MEI1$AS, MEI1$AS == -999, NA)
MEI1$SO <- replace(MEI1$SO, MEI1$SO == -999, NA)
MEI1$ON <- replace(MEI1$ON, MEI1$ON == -999, NA)
MEI1$ND <- replace(MEI1$ND, MEI1$ND == -999, NA)
tail(MEI1)
```
...
for (i in 1:ncol(MEI1)) {
  col <- colnames(MEI1)[i]
  MEI1[[col]] <- replace(MEI1[[col]], MEI1[[col]] == -999, NA)
}
...
MEI1[] <- lapply(MEI1, function(x) replace(x, x == -999, NA))

tail(MEI1)
```

How do the two versions of the cleaned data compare to each other? Are there any major changes? To answer these questions, we can do a quick subtraction of one dataset from another, ignoring the first column of each.
```{r}
MEIdiff <- MEI2[,-1] - MEI1[,-1]
max(MEIdiff, na.rm = T)
min(MEIdiff, na.rm = T)
```

Since there aren't any major discrepancies, let's use the most recent version of the dataset moving forward.
```{r}
MEI <- MEI2
```
...
As you can see, the year column is distorting the y-axis, so let's remove it by indexing into the columns and removing the first column.
```{r}
boxplot(MEI[,-1])
```

Add x-axis and y-axis labels to make it more readable.
```{r}
...
Let's make a quick line graph showing the Multivariate ENSO Index over time!
```{r}
